...
7. **每次 batch_design 后自动保存**：Pencil 不支持自动保存，用 CLI 命令自动写入磁盘，不依赖手动 Cmd+S：
   ```bash
   echo "save(); exit()" | pencil interactive -i <文件路径> -o <文件路径>
   ```
8. **撤销/重做功能有限**：不要依赖撤销回退，重大操作前先保存/commit

### 自动保存（每次 batch_design 后执行）

**重要**：Pencil 官方不支持自动保存（「Auto-save is not yet available - save frequently!」）。依赖手动 Cmd+S 容易遗忘，导致 `batch_design` 已成功的更改未写入磁盘。

**解决方案**：用 Pencil CLI 在 headless 模式下自动保存：

```bash
echo "save(); exit()" | pencil interactive -i specs/sprint-0/prototype.pen -o specs/sprint-0/prototype.pen
```

CLI 会打开文件、执行 `save()`、写入磁盘、退出。每次 `batch_design` 后立即执行，确保更改持久化。

**安装 CLI**：`npm install -g @pencil.dev/cli`（首次使用需要先 `pencil login` 认证）。

**开始工作前必须先保存**

`/new` 创建的临时文档（未保存到磁盘）每次重新序列化时会重新分配节点 ID，导致之前记录的 ID 全部失效。

- 如果 `batch_design` 操作发生回滚，临时文档的 ID 会变化
- 一旦保存到 `.pen` 文件，ID 就固定了，不会变化

**工作流**：`/new` → 立即 Cmd+S 保存为 `.pen` 文件 → 开始 `batch_design` → 每批操作后自动保存。不要在未保存的临时文档上做操作。
...
| 工具 | 用途 | 场景 |
|------|------|------|
| **MCP**（桌面应用） | 细粒度操作，实时预览 | Claude Code 日常设计：`batch_design`、`batch_get`、`get_screenshot` |
| **CLI**（命令行） | 自动保存、批量生成、导出图片 | 保存更改、CI/CD、一次性生成多个原型 |

**日常流程**：MCP 做设计 → CLI 自动保存 → MCP 继续修改。

---

## Pencil CLI 完整参考

CLI 是独立命令行工具，运行和桌面应用相同的编辑器引擎，但完全 headless（无需 GUI）。支持所有 MCP 工具，可以直接在终端调用。
...
# 首次登录（二选一）
pencil login              # 交互式登录（邮箱+密码 或 邮箱+OTP）
# 或
export PENCIL_CLI_KEY=pencil_cli_...  # CI/CD 密钥，优先级高于登录会话
```

登录会话存储在 `~/.pencil/session-cli.json`，与桌面应用分开。

### CLI 的三种模式

#### 1. Agent Mode — Prompt 生成/修改设计

用自然语言 prompt 让 AI 生成或修改设计：
...
# 修改已有设计
pencil --in dashboard.pen --out dashboard-v2.pen --prompt "Add a sidebar navigation"

# 指定模型
pencil --out simple.pen --model claude-haiku-4-5 --prompt "Create a 404 page"

# 生成后导出为图片
pencil --out onboarding.pen --prompt "3-step onboarding flow" --export onboarding.png --export-scale 2
```

| 选项 | 说明 |
|------|------|
| `--in, -i <path>` | 输入 .pen 文件（省略则从空白画布开始） |
| `--out, -o <path>` | 输出 .pen 文件路径 |
| `--prompt, -p <text>` | AI agent 的 prompt |
| `--model, -m <id>` | 使用的模型（默认 `claude-opus-4-6`） |
| `--export, -e <path>` | 生成后导出为图片（png/jpeg/webp/pdf） |
| `--export-scale <n>` | 导出缩放倍数（默认 1） |
| `--verbose-mcp` | 打印完整 MCP 错误详情 |

可用模型：`claude-opus-4-6`（默认）、`claude-sonnet-4-6`、`claude-haiku-4-5`。

#### 2. Interactive Mode — 直接调用 MCP 工具

交互式 shell，可以直接调用所有 MCP 工具（和 Claude Code 里用的一样）：

```bash
# Headless 模式（无 GUI，适合脚本）
...
pencil > batch_design({ operations: 'hero=I(document,{type:"frame",name:"Hero",width:1440,height:900,fill:"#0A0A0A"})' })
pencil > get_screenshot({ nodeId: "hero" })
pencil > save()
pencil > exit()
```

**自动保存管道**（非交互式，一行命令完成保存）：

```bash
echo "save(); exit()" | pencil interactive -i specs/sprint-0/prototype.pen -o specs/sprint-0/prototype.pen
```

CLI 打开文件 → 执行 `save()` → 写入磁盘 → 退出。每次 `batch_design` 后自动执行，确保更改持久化。

#### 3. Batch Mode — 批量处理

通过 JSON 文件批量生成/修改多个设计，任务按顺序执行：
...
每个任务支持：`out`（输出路径，必填）、`prompt`（必填）、`in`（输入路径）、`model`（模型覆盖）。

### CLI 支持的全部 MCP 工具

| 分类 | 工具 | 说明 |
|------|------|------|
| **设计操作** | `batch_design` | 插入、更新、删除、移动、复制、替换节点 |
| | `batch_get` | 按模式或 ID 搜索/读取节点 |
| | `get_variables` | 读取设计变量 |
| | `set_variables` | 更新设计变量 |
| | `get_editor_state` | 获取文档元数据和结构 |
| | `snapshot_layout` | 获取布局结构和计算边界 |
| | `find_empty_space_on_canvas` | 查找画布可用空间 |
| | `search_all_unique_properties` | 递归搜索节点树中的唯一属性 |
| | `replace_all_matching_properties` | 递归替换匹配的属性 |
| **视觉操作** | `get_screenshot` | 渲染节点为 PNG |
| | `export_nodes` | 导出节点为 PNG/JPEG/WEBP/PDF |
| **图片生成** | `G(nodeId, "ai", prompt)` | AI 生成图片（batch_design 内的 G() 操作） |
| | `G(nodeId, "stock", keywords)` | Unsplash 库存图片 |
| **风格指南** | `get_guidelines` | 加载设计指南和样式 |

### 导出图片

```bash
# 导出整个设计
pencil --in design.pen --export output.png

# 指定格式和缩放
pencil --in design.pen --export output@2x.png --export-scale 2 --export-type webp
```
...
| 变量 | 说明 |
|------|------|
| `PENCIL_CLI_KEY` | CLI API 密钥（CI/CD 专用） |
...
```
1. batch_get 读取当前状态 → 2. 确认目标节点/属性 → 3. U()/I() 精确修改 → 4. get_screenshot 验证 → 5. CLI 自动保存
```

- 每次只改一个区域（左栏/右栏/顶部栏），不要一次性改全屏
- 每批操作 ≤ 25 个，避免绑定变量过期
- 每批操作后立刻 `get_screenshot` 验证，发现问题立刻修
- 验证通过后**必须**用 CLI 自动保存，确保更改写入磁盘
...
12. **Pencil 无自动保存** → 每次 `batch_design` 后用 CLI 自动保存，不要依赖手动 Cmd+S。命令：`echo "save(); exit()" | pencil interactive -i <file> -o <file>`
