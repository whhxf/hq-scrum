...
**如果 `/shape` 不可用**（Impeccable 未安装）：必须用纸面回答以上 6 个问题，再进入下一步。不可跳过。

#### 2. 建立设计上下文（Impeccable `/impeccable teach`）

运行 `/impeccable teach`。它会问品牌定义、受众画像、视觉参考，生成 `.impeccable.md`。后续所有设计命令自动读取这个文件。
...
#### 5. 自动化 UX 评审（Impeccable `/critique`）
