...
#### 1. UX 发现（Impeccable `/shape`，推荐）

PRD 定义了"做什么"，但没有定义"怎么做感觉对"。在动手画原型之前，先跑一次 `/shape`。
...
**为什么不是直接画原型**：SubtitleScribe 项目的教训——120 个测试通过后才发现 UI 不可用。因为没人问过"用户在这里是什么心态"、"空状态应该显示什么"。`/shape` 补了 PRD 到原型之间的思考空白。

> 如果不想用 Impeccable，至少用纸面回答以上 6 个问题，再进入下一步。

#### 2. 建立设计上下文（Impeccable `/impeccable teach`，可选）

如果项目安装了 Impeccable，在此阶段运行 `/impeccable teach`。它会问品牌定义、受众画像、视觉参考，生成 `.impeccable.md`。后续所有设计命令自动读取这个文件。
...
#### 5. 自动化 UX 评审（Impeccable `/critique`，可选）

如果项目安装了 Impeccable，原型截图出来后跑 `/critique [页面描述]`。它会：
- 检测 AI Slop（紫色渐变、玻璃拟态、通用卡片网格等）
- 评分 Nielsen 10 条启发式规则
- 检查认知负荷（每个决策点可见选项是否 >4）
- 输出 3-5 个优先修复项

**注意**：`/critique` 针对的是已完成的页面。Sprint 0 阶段用它做初步扫描，不追求高分，只求发现没想到的问题。
...
**为什么需要 Sprint 0**：ASCII 原型发现不了间距、排版、色彩对比度等视觉问题，Pencil 生成的真实截面图成本极低（几分钟对话式出图），但能暴露 ASCII 发现不了的问题。

### 工具参考

- Pencil MCP 操作手册见 `docs/pencil-playbook.md`，所有操作已逐项测试验证
- 设计约束规范见 CLAUDE.md 中的"设计约束"章节，从 Sprint 0 起生效
